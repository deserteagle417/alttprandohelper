(function (window) {
    ...
    function launch_tracker() {
        var params = {
            ...
            bg: document.getElementById('background-color').value || document.getElementById('custom-color').value
        };
        var size = {
        ...
        open('tracker.html?' + query(params), '', 'width={width},height={height},titlebar=0,menubar=0,toolbar=0,scrollbars=0,resizable=0'.replace('{width}', size.w).replace('{height}', size.h));
        setTimeout('window.close()', 5000);
    }

    function query(params) {
        return _.compact(['mode=' + params.mode, params.hmap && 'hmap', params.vmap && 'vmap', params.mode === 'keysanity' && params.ipbj && 'ipbj', params.mode === 'keysanity' && params.podbj && 'podbj', params.sprite && 'sprite=' + params.sprite, !['', 'black'].includes(params.bg) && 'bg=' + encodeURIComponent(params.bg)]).join('&');
    }

    function background_color_changed(event) {
        var value = event.target.value;
        var custom = document.getElementById('custom-color');
        var valid = valid_css_color(custom.value);
    ...
    function valid_css_color(text) {
        ...
        e2.style.color = text;
        return e1.style.color.replace(/ /g, '') !== 'rgb(0,0,0)' || e2.style.color.replace(/ /g, '') !== 'rgb(255,255,255)';
    };

    window.start = function () {
        document.querySelectorAll('.launch').forEach(function (launch) {
            launch.addEventListener('click', launch_tracker);
        });

        var background_color = document.getElementById('background-color');
        var custom_color = document.getElementById('custom-color');
